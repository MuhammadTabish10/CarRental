...
public class VehicleUI extends BaseUI
{
    private final VehicleService vehicleService = new VehicleService();
    private final String[] columnNames = { "Id", "Name", "Model", "Brand", "Color", "Vehicle Owner"};
    private String[][] data;
    ...
    private void initializeUI() {
        frame = new JFrame("Vehicle Dashboard");
    ...
    private void loadTableData() {
        //data = vehicleService.getAllVehicleForJTable();
        data = vehicleService.getAllVehicleWithOwnerForJTable();
    ...
    private void addListeners() {
        searchTF.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent e) {
                String[][] searchData = vehicleService.searchByName(searchTF.getText());
        ...
        updateBtn.addActionListener(event -> {
            if (selectedRecord != null) {
                ...
                Vehicle vehicle = vehicleService.getById(customerId);
                // Open the update form with the selected customer's data
                frame.dispose();
                new EditVehicleUI(vehicle);
            } else {
        ...
        deleteBtn.addActionListener(event -> {
            if (selectedRecord != null) {
                ...
                if (choice == JOptionPane.YES_OPTION) {
                    ...
                    if(vehicleService.hasBooking(Long.parseLong(selectedRecord[0]))){
                        JOptionPane.showMessageDialog(frame,"This vehicle cannot be deleted because it's has booked");
                    }
                    else{
                        // Perform the deletion operation in the database through CustomerService
                        vehicleService.deleteById(Long.valueOf(selectedRecord[0])); // Assuming the first column contains the ID
                        // Remove the row from the table's DefaultTableModel
                        dtm.removeRow(table.getSelectedRow());
                        // Reset the selectedRecord variable and disable the delete button
                        JOptionPane.showMessageDialog(frame, "Vehicle deleted", "Success", JOptionPane.INFORMATION_MESSAGE);
                        selectedRecord = null;
                        deleteBtn.setEnabled(false);
                    }
                }
