...
public class VehicleOwnerUI extends BaseUI
{
    private final VehicleOwnerService vehicleOwnerService = new VehicleOwnerService();
    private final String[] columnNames = { "Id", "Name", "Phone", "Cnic", "Address", "Commission"};
    private String[][] data;
    ...
    private void loadTableData() {
        data = vehicleOwnerService.getAllVehicleOwnerForJTable();
    ...
    private void addListeners() {
        searchTF.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent e) {
                String[][] searchData = vehicleOwnerService.searchByName(searchTF.getText());
        ...
        updateBtn.addActionListener(event -> {
            if (selectedRecord != null) {
                ...
                VehicleOwner vehicleOwner = vehicleOwnerService.getById(customerId);
                // Open the update form with the selected customer's data
                frame.dispose();
                new EditVehicleOwnerUI(vehicleOwner);
            } else {
        ...
        deleteBtn.addActionListener(event -> {
            if (selectedRecord != null) {
                ...
                if (choice == JOptionPane.YES_OPTION) {
                    ...
                    if(vehicleOwnerService.hasBooking(Long.parseLong(selectedRecord[0]))){
                        JOptionPane.showMessageDialog(frame,"This vehicle owner cannot be deleted because he has bookings");
                    }
                    else{
                        // Perform the deletion operation in the database through CustomerService
                        vehicleOwnerService.deleteById(Long.valueOf(selectedRecord[0])); // Assuming the first column contains the ID
                        // Remove the row from the table's DefaultTableModel
                        dtm.removeRow(table.getSelectedRow());
                        // Reset the selectedRecord variable and disable the delete button
                        JOptionPane.showMessageDialog(frame, "Vehicle Owner deleted", "Success", JOptionPane.INFORMATION_MESSAGE);
                        selectedRecord = null;
                        deleteBtn.setEnabled(false);
                    }
                }
